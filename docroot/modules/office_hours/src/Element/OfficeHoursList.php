  ...
  public static function validateOfficeHoursSlot(&$element, FormStateInterface $form_state, &$complete_form) {
    ...
    if ($input_exists) {
      ...
      elseif (!empty($limitstart) || !empty($limitend)) {
        if (($start && ($limitstart * 100) > $start) || ($end && ($limitend * 100) < $end)) {
          $error_text = 'Hours are outside limits ( @start - @end ).';
        }
      }

//        if ($hour < 0 || $hour > 23) {
//          $error_text = t('Hours should be between 0 and 23.', array(), array('office_hours'));
//          $form_state->setErrorByName('office_hours_select', $error_text);
//        }
//        if ($minute < 0 || $minute > 59) {
//          $error_text = t('Minutes should be between 0 and 59.', array(), array('office_hours'));
//          $form_state->setErrorByName('office_hours_select', $error_text);
//        }
